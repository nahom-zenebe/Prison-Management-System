import medicalRecordManagment from "../model/medical"
...
export const AddMedicalRecord=async(req,res)=>{
    try {
        ...
        if (!recordId || !inmateId || !diagnosis || !treatment || !recordData) {
          ...
          const newMedicalRecord = new medicalRecordManagment({
            recordId,
            inmateId,
            diagnosis,
            treatment,
...
export const AllMedicalRecord=async(req,res)=>{
    ...
    try {
        const allmedicaldata=await medicalRecordManagment.find({})
        if(!allmedicaldata){
...
export const getMedicalRecordById = async (req, res) => {
    try {
      ...
      const medicalRecord = await medicalRecordManagment.findById(Medicalid);
